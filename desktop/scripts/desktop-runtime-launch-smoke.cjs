...
function requestText(targetUrl, { timeoutMs = 5_000 } = {}) {
  return new Promise((resolve, reject) => {
...
function findLoadedDesktopPage(targets, startUrl) {
  ...
  try {
    ...
    const loadedPage = await waitForCheck(
      async () => {
        ...
        return findLoadedDesktopPage(JSON.parse(response.body), startUrl);
      },
